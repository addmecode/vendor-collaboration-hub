...
`AMC Telemetry` wraps `Session.LogMessage` and always emits these dimensions:
...
dimension names. One resource is what makes an end-to-end trace possible: a single
query can show the link being issued in BC, opened in the browser, forwarded by the
Function and validated in AL.

`vchCorrelationId` is generated by the page per user action, passed through the
Function, sent in the payload, stored on the proposal, and repeated on every
telemetry event on both sides — so one KQL query reconstructs a whole negotiation
across the page, the Function and BC.

```kusto
traces
| where customDimensions.eventId startswith "AMC"
| where customDimensions.vchCorrelationId == "8f2c..."
| project timestamp, message, customDimensions.vchEventId,
          customDimensions.vchResult, customDimensions.vchDurationMs
...
| # | Task | Delivers | See it work | Status |
|---|---|---|---|---|
| 23 | **Telemetry and dashboard** | Every event id of §10 emitted on both sides; `vchCorrelationId` threaded from the page through the Function into AL; `docs/architecture/telemetry-queries.md` with the dashboard queries | Run one negotiation, then one KQL query returning it in order from `VCH0120` to `VCH0220`, across page, Function and BC | |
