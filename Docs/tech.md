...
Creating an `AMC Vendor Request` or `AMC Vendor Request Line` directly from its
List, Document or ListPart page is not a supported business path. Those pages set
`InsertAllowed = false`; only `AMC Request Mgt.CreateFromOrder` creates the request
and its snapshot lines. This keeps the request tied to its purchase order and
prevents bypassing the active-request invariant and its header lock.
